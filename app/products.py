...
def products():
    ...
    # Get category by ID
    result = cur.execute('SELECT products.is_active, products.title, products.id, users.name AS created_by, categories.title as category, products.modality FROM products INNER JOIN users INNER JOIN categories ON products.created_by=users.id AND products.category=categories.id ORDER BY category ASC')
...
def add_product():
    ...
    if request.method == 'POST' and form.validate():
        ...
        cur.execute('INSERT INTO products(is_active, created_by, title, category, rooms, bathrooms, area, price, cond_fare, iptu_fare, modality, street, district, city, state, description) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)', (is_active, created_by, title, category, rooms, bathrooms, area, price, cond_fare, iptu_fare, modality, street, district, city, state, description))
...
def edit_product(id):
    ...
    if request.method == 'POST':
        ...
        if form.validate():
            ...
            # Execute
            cur.execute('UPDATE products SET is_active=%s, title=%s, category=%s, rooms=%s, bathrooms=%s, area=%s, price=%s, cond_fare=%s, iptu_fare=%s, modality=%s, street=%s, district=%s, city=%s, state=%s, description=%s WHERE id=%s', (is_active, title, category, rooms, bathrooms, area, price, cond_fare, iptu_fare, modality, street, district, city, state, description, id))
