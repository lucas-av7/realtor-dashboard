...
def images(product_id):
    ...
    try:
        ...
        flash('Imóvel selecionado não existe.', 'danger')
        return redirect(url_for('products.products'))

    # Get images by product ID
    result = cur.execute(
        'SELECT products.id AS product_id, products.title AS product_title, images.id AS image_id, images.main, images.url, images.url_thumb, images.delete_url FROM products INNER JOIN images ON products.id=images.product_id WHERE product_id=%s ORDER BY images.id ASC', [product_id])
    ...
    if request.method == 'POST':
        ...
        for pic in pics:
            ...
            photos = cur.execute(
                'SELECT * FROM images WHERE product_id=%s and main=True', [product_id])
            main = photos == 0

            cur.execute('INSERT INTO images(main, product_id, url, url_thumb, url_medium, delete_url) VALUES (%s, %s, %s, %s, %s, %s)',
                        (main, product_id, url, url_thumb, url_medium, delete_url))
